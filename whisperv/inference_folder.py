...
def _merge_similar_identities_by_avatar(annotated_tracks, avatars_cache_path: str,
                                        sim_thresh: float = 0.55, max_overlap_frames: int = 1):
    """Merge visually similar identities if their time spans do not overlap."""
    try:
        import pickle
        import numpy as _np
        import torch
...
def _id_color_map_global(tracks_list):
    ...
    for idx, ident in enumerate(uniq):
        colors[ident] = base_palette[idx % n_palette]  # RGB tuple for Skia
    return colors
...
def _render_panel_chunk_worker(args_pack):
    ...
    font_title = skia.Font(typeface, int(round(20 * font_scale)))
    font_text = skia.Font(typeface, int(round(18 * font_scale)))
    ...
    card_gap = int(round(10 * layout_scale))
...
def _render_side_panel_skia_parallel(base_video_path: str,
    ...
    wrap_size = int(round(18 * font_scale))
    for m in msgs:
