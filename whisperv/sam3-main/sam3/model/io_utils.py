    ...
    if video_loader_type == "cv2":
        ...
        if not async_loading_frames:
            ...
            if async_thread is not None:
                async_thread.join()
        return lazy_images, lazy_images.video_height, lazy_images.video_width
    elif video_loader_type == "pyav":
        # Lightweight PyAV-based loader that keeps frames on CPU and decodes
        # on demand, avoiding loading the full video tensor into GPU memory.
        try:
            import av  # type: ignore
        except Exception as e:  # pragma: no cover - env-specific
            raise RuntimeError("video_loader_type='pyav' requires the 'av' package") from e
        ...
        vstream = vstreams[0]
        video_height = int(vstream.codec_context.height)
        video_width = int(vstream.codec_context.width)
        if vstream.frames and int(vstream.frames) > 0:
            num_frames = int(vstream.frames)
        else:
            num_frames = sum(1 for _ in container.decode(video=vstream.index))
        container.close()
        if num_frames <= 0:
            raise RuntimeError(f"PyAV reported non-positive frame count for {video_path}")

        class AsyncVideoFileLoaderWithPyAV:
            """
            Minimal async-style loader using PyAV.

            Behaves similarly to AsyncImageFrameLoader: exposes __len__/__getitem__
            and returns normalized float16 tensors (C,H,W) on CPU. SAM3 will move
            per-frame tensors to CUDA inside _get_img_feats.
            """

            def __init__(self, path, size, img_mean_fp16, img_std_fp16, n_frames):
                self.video_path = path
                self.image_size = int(size)
                self.num_frames = int(n_frames)
                self.img_mean = img_mean_fp16  # 3x1x1 float16
                self.img_std = img_std_fp16    # 3x1x1 float16
            ...
            def _reset_decoder(self):
                ...
                if not vstreams:
                    ...
                    raise RuntimeError(f"No video stream found during decode in {self.video_path}")
                self._vstream = vstreams[0]
                self._iter = self._container.decode(video=self._vstream.index)
                self._next_idx = 0
            ...
            def __getitem__(self, idx):
                ...
                if idx < 0 or idx >= self.num_frames:
                    raise IndexError(
                        f"AsyncVideoFileLoaderWithPyAV index {idx} out of range 0..{self.num_frames-1}"
                    )
                # SAM3 video pipeline accesses frames in increasing order.
                ...
                while self._next_idx <= idx:
                    try:
                        ...
                        raise RuntimeError(
                            f"AsyncVideoFileLoaderWithPyAV exhausted at index {self._next_idx} < requested {idx}"
                        ) from e
                    self._next_idx += 1
                if frame is None:
                    raise RuntimeError(f"AsyncVideoFileLoaderWithPyAV failed to decode frame {idx}")
                frame_rgb = frame.to_rgb().to_ndarray()  # H,W,3 uint8
                img = Image.fromarray(frame_rgb)
                img = TF.resize(img, size=(self.image_size, self.image_size))
                img_t = TF.to_tensor(img).to(dtype=torch.float16)  # C,H,W in [0,1]
                img_t -= self.img_mean
                img_t /= self.img_std
                return img_t

        img_mean_fp16 = torch.tensor(img_mean, dtype=torch.float16)[:, None, None]
        img_std_fp16 = torch.tensor(img_std, dtype=torch.float16)[:, None, None]
        lazy_images = AsyncVideoFileLoaderWithPyAV(
            video_path,
            image_size,
            img_mean_fp16,
            img_std_fp16,
        ...
        return lazy_images, video_height, video_width
    else:
        raise RuntimeError("video_loader_type must be either 'cv2', 'torchcodec', or 'pyav'")
